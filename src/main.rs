use srvcs_range::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-range listening");
    let deps = Deps {
        sortascending_url: cfg.sortascending_url.clone(),
